...
(* Christian Szablewski-Paz and Shreyas Keerthi *)
...
let rec prune n t =
  ...
  | Node(d, lt, rt) -> 
    if n = 0 then 
      ...
      Node(d, prune (n - 1) lt, prune (n - 1) rt) 
...
let perfect t =
  ...
  | Empty -> true
  | Node(d, lt, rt) -> 
  if size (paths_to_leaves rt) = size (paths_to_leaves lt) && height rt = height lt && size rt = size lt
    then true
  else false
